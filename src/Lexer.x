...
<0> BEGIN        { mkl TBEGIN     }
<0> END          { mkl TEND       }
<0> __ENCODING__ { mkl T_ENCODING }
<0> __END__      { mkl T_END      }
<0> __FILE__     { mkl T_FILE     }
<0> __LINE__     { mkl T_LINE     }
<0> alias        { mkl TAlias     }
<0> and          { mkl TAnd       }
<0> begin        { mkl TBegin     }
<0> break        { mkl TBreak     }
...
<0> class        { mkl TClass     }
<0> def          { mkl TDef       }
<0> defined\?    { mkl TDefined   }
<0> do           { mkl TDo        }
<0> else         { mkl TElse      }
<0> elsif        { mkl TElsif     }
<0> end          { mkl TEnd       }
<0> ensure       { mkl TEnsure    }
<0> false        { mkl TFalse     }
<0> for          { mkl TFor       }
<0> if           { mkl TIf        }
<0> in           { mkl TIn        }
<0> module       { mkl TModule    }
<0> next         { mkl TNext      }
<0> nil          { mkl TNil       }
<0> not          { mkl TNot       }
<0> or           { mkl TOr        }
<0> redo         { mkl TRedo      }
<0> rescue       { mkl TRescue    }
<0> retry        { mkl TRetry     }
