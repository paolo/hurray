...
statement
        :   body:statementWithoutModifier
                                    ( !IF_MODIFIER^     if_condition:expression 
                                    | !UNLESS_MODIFIER^ unless_condition:expression 
                                    | !WHILE_MODIFIER^  while_condition:expression  
                                    | !UNTIL_MODIFIER^  until_condition:expression  
                                    | !RESCUE_MODIFIER^ rescue_condition:expression 
                                    )*
        ;
...
statementWithoutModifier
        ...
        |   expression (options:COMMA! (seen_star:REST_ARG_PREFIX)? (mlhs_item)? )* 
                       ( (ASSIGN! | ASSIGN_WITH_NO_LEADING_SPACE!) (options:LINE_BREAK!)? mrhs)?
        |   REST_ARG_PREFIX mlhs_item (ASSIGN! | ASSIGN_WITH_NO_LEADING_SPACE!) (options:LINE_BREAK!)? mrhs
        ;
...
mlhs_item
        :   (lparen dotColonOrArrayAccess COMMA) => 
             lparen dotColonOrArrayAccess (COMMA! (seen_star:REST_ARG_PREFIX)? dotColonOrArrayAccess)+ RPAREN!   
        |   dotColonOrArrayAccess
        ;

mrhs
        :   expression  (COMMA! (seen_star:REST_ARG_PREFIX)? expression)*
...
undefStatement
        :   "undef"^ (options:LINE_BREAK!)? undef_parameter (COMMA! undef_parameter)*
        ;

aliasStatement
        :   "alias"^ (options:LINE_BREAK!)? ( (GLOBAL_VARIABLE (LINE_BREAK!)? GLOBAL_VARIABLE) | (undef_parameter (LINE_BREAK!)? undef_parameter) )
        ;

//TODO should be mlhs
block_vars
        :   mlhs_item (COMMA! (seen_star:REST_ARG_PREFIX)? (mlhs_item)?)*
        |   REST_ARG_PREFIX (dotColonOrArrayAccess)?
        ;

codeBlock
        :   "do"! (LINE_BREAK!)? blockContent "end"!
        |   LCURLY_BLOCK! blockContent RCURLY!
        ;

protected
blockContent
        :   (BOR (block_vars)? BOR! | LOGICAL_OR)? (compoundStatement)?
        ;

symbol
        :   COLON_WITH_NO_FOLLOWING_SPACE!
                    (IDENTIFIER (options:ASSIGN_WITH_NO_LEADING_SPACE)?
                    |FUNCTION   (options:ASSIGN_WITH_NO_LEADING_SPACE)?
                    |CONSTANT   (options:ASSIGN_WITH_NO_LEADING_SPACE)?
                    |GLOBAL_VARIABLE 
                    |INSTANCE_VARIABLE
                    |CLASS_VARIABLE
...
//and or
andorExpression
        :   notExpression (options:( ("and"^ (options:LINE_BREAK!)?) | ("or"^ (options:LINE_BREAK!)?) ) notExpression)*
        ;

//not
notExpression
        :   "not"^ (options:LINE_BREAK!)? notExpression
        |   ternaryIfThenElseExpression
        ;

ternaryIfThenElseExpression
        :   assignmentExpression (QUESTION^ (LINE_BREAK!)? ternaryIfThenElseExpression (COLON! | COLON_WITH_NO_FOLLOWING_SPACE!) (LINE_BREAK!)? ternaryIfThenElseExpression)?
        ;
...
assignmentExpression
        :   rangeExpression
            (   (   ASSIGN^             (LINE_BREAK!)?
                |   ASSIGN_WITH_NO_LEADING_SPACE^ (LINE_BREAK!)?
                |   PLUS_ASSIGN^        (LINE_BREAK!)?
                |   MINUS_ASSIGN^       (LINE_BREAK!)?
                |   STAR_ASSIGN^        (LINE_BREAK!)?
                |   DIV_ASSIGN^     (LINE_BREAK!)?
                |   MOD_ASSIGN^     (LINE_BREAK!)?
                |   POWER_ASSIGN^       (LINE_BREAK!)?
                |   BAND_ASSIGN^        (LINE_BREAK!)?
                |   BXOR_ASSIGN^        (LINE_BREAK!)?
                |   BOR_ASSIGN^     (LINE_BREAK!)?
                |   LEFT_SHIFT_ASSIGN^  (LINE_BREAK!)?
                |   RIGHT_SHIFT_ASSIGN^ (LINE_BREAK!)?
                |   LOGICAL_AND_ASSIGN^ (LINE_BREAK!)?
                |   LOGICAL_OR_ASSIGN^  (LINE_BREAK!)?
                )
            ternaryIfThenElseExpression)*
        ;
...
//||
logicalOrExpression
        :   logicalAndExpression (LOGICAL_OR^ (LINE_BREAK!)? logicalAndExpression)*
        ;

//&&
logicalAndExpression
        :   equalityExpression (LOGICAL_AND^ (LINE_BREAK!)? equalityExpression)*
        ;
